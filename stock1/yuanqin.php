...
$name1=@$_COOKIE['name'];
if ($name=='') {$name=$name1;}
echo '<html lang="zh-CN"> <meta charset="utf-8" http-equiv="Content-Type" content="text/html;"><title>远勤论坛</title><head><link rel="shortcut icon" href="/favicon.ico">';
...
?>
<h2 style="color:red" align="center">远勤论坛</h2><small><br/><a style="color:#A1A1A1;" href="#liuyan">留言</a><br/></small><br/>
<?php
if ($name!=""){
...
"<a onclick= 'if(confirm(\"是否确定删除！\")==false)return false;' style='font-size:9pt;color:#A1A1A1;' target='_blank' href='/stock1/del.php?id=$arr[0]'>[删除]</a>";
	...
	echo '&nbsp';
      ...
      echo '<form method="POST" action="/stock1/yuanqin.php"><div id="liuyan" align="center"><input type="text" name=topic placeholder="请输入标题" style="width:400px;height:35px;font-size:18px;vertical-align:middle ;float left;"/> ';
	...
	echo '<p>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp';
      echo '<input type="submit"  style="text-align:center;margin:auto;width:70px;height:25px;font-size:18px;line-height:20px;" align="center"  onclick="refresh()" value="提交" name="B1">';
      echo '&nbsp&nbsp </form>';
      echo '</p>';
      $topic=@$_POST['topic'];
	$message=str_replace('
	...
	$arr2=mysql_fetch_array($result2);
     if($message==""){
      ...
      //   echo "<script>alert('您的留言重复了！或者不要重复提交。')</script>";
         ...
         if ($result4)           echo "<script>window.location.replace('/stock1/yuanqin.php')</script>"; else echo "留言失败，可能服务器暂时不响应，请返回重试！";
