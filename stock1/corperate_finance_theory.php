...
$page='corperate_finance_theory';

$name=@$_GET['name'];
$name1=@$_COOKIE['name'];
if ($name=='') {$name=$name1;}
echo '<html lang="zh-CN"> <meta charset="utf-8" http-equiv="Content-Type" content="text/html;"><title>财务理论专题（本科）</title><head><link rel="shortcut icon" href="/favicon.ico">';
...
?>
<h2 style="color:red" align="center">财务理论专题（本科）</h2><br/>
<?php
if ($name!=""){
	...
	echo "条。</p>";
     while ($arr=mysql_fetch_array($result)){
		...
		if ($name === $arr[1] and $name!='Jiangang'){
			echo
"<a onclick= 'if(confirm("是否确定！")==false)return false;' style='font-size:9pt;color:#A1A1A1;' target='_blank' href='/stock1/del.php?id=$arr[0]'>[删除]</a>";
...
"<a onclick= 'if(confirm("是否确定！")==false)return false;' style='font-size:9pt;color:#A1A1A1;' target='_blank' href='/stock1/del.php?id=$arr[0]'>[删除]</a>";
	...
	echo '<p>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp';
      echo '<input type="submit"  style="text-align:center;margin:auto;width:60px;height:25px;font-size:25px;line-height:20px;" align="center"  onclick="refresh()" value="提交" name="B1">';
      echo '&nbsp&nbsp <input  style="text-align:center;margin:auto;width:60px;height:25px;font-size:25px;line-height:20px;" align="center" type="reset" value="重置" name="B2"></form>';
      echo '</p>';
      $topic=@$_POST['topic'];
	$message=str_replace('
','<br/>',@$_POST['message']);
