...
$page='investment_undergraduate';

$name=@$_GET['name'];
$name1=@$_COOKIE['name'];
if ($name=='') {$name=$name1;}
echo '<html lang="zh-CN"> <meta charset="utf-8" http-equiv="Content-Type" content="text/html;"><title>投资学（本科）</title><head><link rel="shortcut icon" href="/favicon.ico">';
...
?>
<h2 style="color:red" align="center">投资学（本科）</h2><small><br/><a style="color:#A1A1A1;" href="#liuyan">留言</a><br/></small><br/>
<?php
if ($name!=""){
	...
	$arr2=mysql_fetch_array($result2);
     if($message==""){
      ...
      //   echo "<script>alert('您的留言重复了！或者不要重复提交。')</script>";
         ...
         if ($result4){echo "<script>window.location.replace('";echo $page;echo ".php'</script>"; }else echo "留言失败，可能服务器暂时不响应，请返回重试！";
     }
