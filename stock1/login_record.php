...
echo '<div align="center"><h2 align="center" style="color:red">登录记录</h2>';
echo '<br/><br/>';


if ($name==='建刚'){
	$result=mysql_query("SELECT * FROM r0gbqnbc4x.log order by time_at desc limit 100;");
	echo "<table  width='70%'  cellspacing='0'  border='1' bordercolor='#D0D0D0' cellpadding='4' align='center'>";
	echo "<tr><th>编号</th><th>网名</th><th>ip</th><th>时间</th><th>班级</th></tr>";
    $i=1;
    while ($row = mysql_fetch_array($result, MYSQL_NUM)) {	printf("<tr><td>%s</td><td><a target='_blank' href='/stock1/user.php?name=%s'>%s</a></td><td>%s</td><td>%s</td><td>%s</td></td></tr>",$i,$row[1],$row[1],$row[2],$row[3],$row[4]);
        $i++;
	}
