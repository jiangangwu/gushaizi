...
sys.path.append('/users/jiangang/documents/www/stock1/')
from jg import *
...
deleteFile('/users/jiangang/documents/www/股票观察_副本.xlsm')
copyFile('/users/jiangang/documents/www/股票观察.xlsm', '/users/jiangang/documents/www/股票观察_副本.xlsm')

data = xlrd.open_workbook('/users/jiangang/documents/www/股票观察_副本.xlsm')
table = data.sheets()[1]
...
for c in codes[8:]:
    ...
    a =  '<div style = "line-height:0.35"><br/></div><h2  align="center">'+ '<a href="http://' + gswz + '" target="_blank">' + gpjc + '（' + c[0:6] + '）</a></h2><small><br/><a style="color:#A1A1A1;" href="#liuyan">留言</a><br/></small>'
    a = a + '<?php $code="' + c[0:6] + '";'
    a = a + '''
    ...
    a=  a + '<p align="center">' + getDate() + '&nbsp;&nbsp;当天涨跌幅：' + tn(dtzd) +  '%</p>' \
          ...
          + '<a href="http://www.windin.com/home/stock/html/'+ c + '.shtml" target="_blank">&nbsp;&nbsp;万得</a>' \
    ...
    a =a + '<p align="center"><table cellspacing="0"  align="center" border="1" bordercolor="#D0D0D0" cellpadding="4"><tr><td>股票排序</td><td>' + tnn(pm)  \
         ...
         + '</td></tr><tr><td style="word-wrap:break-word;"><a href="/stock1/' + hh['行业1 ' + hym1] + '.php">1. ' + hym1 + '</a></td><td>' + tp(pm1) \
         + '</td><td style="word-wrap:break-word;"><a href="/stock1/' +hh['行业2 ' + hym2] + '.php">2. ' + hym2 + '</a></td><td>' + tp(pm2)\
         + '</td></tr><tr><td style="word-wrap:break-word;"><a href="/stock1/' + hh['行业3 ' + hym3] + '.php">3. ' + hym3 + '</a></td><td>' + tp(pm3) \
         +'</td><td style="word-wrap:break-word;"><a href="/stock1/' + hh['行业4 ' + hym4] + '.php">4. ' +  hym4 + '</a></td><td>' + tp(pm4)+'</td></tr></table>'
    ...
    a = a + '<h4 align="center">（五）利润成长能力得分：' + tn2(lrczdf) + '</h4>' \
          ...
          + '</font></td><td>上半年增长率</td><td>' + tp(bnzzl1532) \
          + '</td</tr><tr><td>去年增长率</td><td>' + tp(lrzz14) \
    ...
    a = a + '<h4 align="center">（六）主营成长得分：' + tn2(zyczdf) + '</h4>' \
          ...
          + '</td><td>去年增长率</td><td>' + tp(zyzz14) \
          + '</td></tr><tr><td>前年增长率</td><td>' + tp(zyzz13) \
    ...
    fl = '/users/jiangang/documents/www/stock1/' + c[0:6] + '.php'
...
bt = '<div style = "line-height:0.35"><br/></div><h2 align="center">预计涨跌排序</h2><p  align="center"><small><br/><a style="color:#A1A1A1;" href="#liuyan">留言</a><br/><br/></small>' + getDate() \
...
for i in range(8,len(pm)):
    nr = nr + '<tr><td align="center" width="3%">' \
            ...
            + gpdm[i][0:6] + '</td><td align="center" width="8%" nowrap="nowrap" ><a  target="_blank" href="/stock1/' + gpdm[i][0:6] + '.php">' \
            ...
            + tn2(dzxs[i]) + '</td><td width="9.5%"  nowrap="nowrap"><a   href="/stock1/' + hh['行业4 ' + hym4[i].replace(' ','')] + '.php" >' \
            + hym4[i].replace(' ','') + '</a></td></tr>'
    if ii == 0:
        if float(yc[i+1]) < 0.1:
            ii = ii + 1
            f = codecs.open('/users/jiangang/documents/www/stock1/px.php','w', 'UTF-8')
            px =  ah.replace('blog',pagename).replace("股筛子",'排序') + hybt1.replace("预计涨跌排序",'<a   href="/stock1/qpx.php" >排序（大于10%）</a>') + nr + '<table><br/><br/></table>' + at
            f.write(px)
            f.close()

f = codecs.open('/users/jiangang/documents/www/stock1/qpx.php','w', 'UTF-8')
qpx =  ah.replace('blog',pagename).replace("股筛子",'全排序') + hybt1.replace("预计涨跌排序",'全部股票排序') + nr + '<table><br/><br/></table>' + at
f.write(qpx)
f.close()
print("        全排序文件完成。")
...
for h in hy1list:
    ...
    for i in range(8,len(pm)):
        if hym1[i] == h:
            nr = nr + '<tr><td align="center" width="3%">' \
                    + tnn(pm[i]) + '</td><td align="center" width="6%" >' \
                    + gpdm[i][0:6] + '</td><td align="center" width="8%" nowrap="nowrap" ><a href="/stock1/' + gpdm[i][0:6] + '.php">' \
                    + gpjc[i] + '</a></td><td align="center" width="5%" >' \
                    + tn2(dtzd[i]) +  '%</td><td align="center" width="7%" >'\
                    + tn(spj[i]) + '</td><td align="center" width="7%" >' \
                    + tn(ttmsy[i]) + '</td><td align="center" width="6.7%" ><font color="Chocolate">' \
                    ...
                    + tn2(hypjdf[i]) + '</td><td align="center" width="6%">' \
                    + tn2(gpdf[i]) + '</td><td align="center" width="6%">' \
                    + tn2(dzxs[i]) + '</td><td width="9.5%"  nowrap="nowrap"><a   href="/stock1/'+ hh['行业2 ' + hym2[i].replace(' ','')] + '.php" >' \
                    + hym2[i].replace(' ','') + '</a></td></tr>'
    nr = ah.replace('blog','行业1 ' + h).replace("股筛子",h)  \
                + hybt1.replace('行业名4','行业名2').replace("预计涨跌排序",'<a   href="/stock1/hyjg.php" >行业1：' + h + '</a>')  \
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/users/jiangang/documents/www/stock1/'+ hh['行业1' + ' ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
for h in hy2list:
    nr = '<p align="center">'
    for i in range(8,len(pm)):
        if hym2[i] == h:
            nr = nr + '<tr><td align="center" width="3%">' \
                    ...
                    + tn2(hypjdf[i]) + '</td><td align="center" width="6%">' \
                    + tn2(gpdf[i]) + '</td><td align="center" width="6%">' \
                    + tn2(dzxs[i]) + '</td><td width="9.5%"  nowrap="nowrap"><a   href="/stock1/' + hh['行业3 ' + hym3[i].replace(' ','')] + '.php" >' \
                    + hym3[i].replace(' ','') + '</a></td></tr>'
    nr = ah.replace('blog','行业2 ' + h).replace("股筛子",h)  \
                + hybt1.replace('行业名4','行业名3').replace("预计涨跌排序",'<a   href="/stock1/hyjg.php" >行业2：' + h + '</a>')  \
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/users/jiangang/documents/www/stock1/' + hh['行业2' + ' ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
for h in hy3list:
    nr = '<p align="center">'
    ...
    nr = ah.replace('blog','行业3 ' + h).replace("股筛子",h)  \
                + hybt1.replace("预计涨跌排序",'<a   href="/stock1/hyjg.php" >行业3：' + h + '</a>')  \
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/users/jiangang/documents/www/stock1/' + hh['行业3' + ' ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
for h in hy4list:
    nr = '<p align="center">'
    ...
    nr = ah.replace('blog','行业4 ' + h).replace("股筛子",h)  \
                + hybt1.replace('行业名4','行业名3').replace("预计涨跌排序",'<a   href="/stock1/hyjg.php" >行业4：' + h + '</a>')  \
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/users/jiangang/documents/www/stock1/' + hh['行业4 ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
print("        各个分行业排序文件完成。")
...
table = data.sheets()[1]
hy = '<div style = "line-height:0.35"><br/></div><h2 align="center"><a href="/stock1/hyjg.php">行业</a></h2><small><br/><a style="color:#A1A1A1;" href="#liuyan">留言</a><br/></small>'+ '<p  align="center">' + getDate() + '</p><p align="center">市场平均泡沫：' + tp(str((float(table.cell(3,3).value) + float(table.cell(3,6).value) + float(table.cell(3,9).value))/3)) + '&nbsp;建议仓位：' + tp(str(table.cell(3,2).value)) + '</p><p  align="center">上证泡沫：' + tp(str(table.cell(3,3).value)) + '&nbsp;中小板泡沫：' + tp(str(table.cell(3,6).value)) +'&nbsp;创业板泡沫：' + tp(str(table.cell(3,9).value))  + '</p>'
hy = hy + '<p align = "center">' + '<a href="http://finance.sina.com.cn" target="_blank">新浪财经</a>' \
        ...
        + '<a href="http://www.stock1star.com/roll/finance.shtml" target="_blank">&nbsp;&nbsp;证券之星</a>' \
        + '<a href="http://finance.eastmoney.com/yaowen.html" target="_blank">&nbsp;&nbsp;东方财富</a>' \
        + '<a href="http://news.cnstock1.com/" target="_blank">&nbsp;&nbsp;中国证券网</a></p>'
...
for i in range(3,len(c1)):
    hy = hy + '<tr><td align="center" width="4%">' \
            + c0[i][-1] + '</td><td align="left" width="3%" ><a  href="/stock1/' + hh[c0[i] + ' ' + c1[i]] + '.php" >' \
            ...
            + tp(c11[i]) + '</td></tr>'
f = codecs.open('/users/jiangang/documents/www/stock1/hy.php','w', 'UTF-8')
hy = ah.replace('blog','hy').replace('股筛子','行业') + '' + hy + '</table>' + '<br/><br/>' + at
f.write(hy)
f.close()
print("        行业排序文件完成。")
...
def wnjian(fname,title, contents,ah,at):
    print("开始写" + title + "文件……")
    topic = '<h2 style="color:red" align="center">' + title + '</h2><small><br/><a style="color:#A1A1A1;" href="#liuyan">留言</a><br/></small>'
    f = codecs.open('/users/jiangang/documents/www/stock1/' + fname + '.php','w', 'UTF-8')
...
wnjian('ai', 'AI','',ah,at)
wnjian('share', 'Share','',ah,at)
wnjian('research', '研究','',ah,at)
wnjian('dissertation_postgraduate', '论文指导','',ah,at)
wnjian('financial_english_undergraduate', '财务专业英语（本科）','',ah,at)
wnjian('investment_undergraduate', '投资学（本科）','',ah,at)
wnjian('investment_postgraduate', '投资学（硕士）','',ah,at)
wnjian('jiangang', '吴建刚简介','',ah,at)