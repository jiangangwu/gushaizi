...
if ($topic==""){
    ...
    $result_topic=mysql_query("select topic from user where course='投资理论与实践（2017冬）';");
    $sign=0;
    ...
    if ($sign==0){
        ...
        if ($result){
            echo "<script>window.location.replace('/stock1/inv_stock_practice.php')</script>";
        }else{
            echo "<html lang='zh-CN'><meta charset='utf-8'><body><a href='/stock1/inv_stock_practice.php' style='text-decoration:none;'>系统出现问题，请重试。</a></body></html>";
        }
    }
}
?>