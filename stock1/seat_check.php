<?php
   ...
   $name=$_COOKIE['name'];
   $seat=$_POST['seat'];
   $result_formername=mysql_query("select name from seats where identifier='$seat';");
   ...
   $result_absence=mysql_query("select absence,absence_time from seats where name='$name';");
   $absence=mysql_fetch_array($result_absence);
    
   mysql_query("UPDATE seats SET seat=identifier,name='',absence=0,absence_time='' where name='$name';");

   $sql = "update seats set name='$name', seat='$username[0]',absence='$absence[0]',absence_time='$absence[1]',  ip='$ip',
           time_at=NOW() where identifier='$seat';";
   $result=mysql_query($sql);
   if ($result){
      echo "<script>window.location.replace('/stock1/seats.php')</script>";
       echo "<html lang='zh-CN'><meta charset='utf-8'><body><a style='text-decoration:none;'
    href='/stock1/seats.php'>，恭喜！选座成功。请点击返回。</a></body></html>";
   }else{
       echo "<html lang='zh-CN'><meta charset='utf-8'><body><a href='/stock1/seats.php' style='text-decoration:none;'>系统出现问题，请重试。</a></body></html>";
   }
   }
?>
