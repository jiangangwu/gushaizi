...
sys.path.append('/users/jiangang/documents/stock1/')
from jg import *
...
deleteFile('/users/jiangang/investments/股票观察_副本.xlsm')
copyFile('/users/jiangang/investments/股票观察.xlsm', '/users/jiangang/investments/股票观察_副本.xlsm')

data = xlrd.open_workbook('/users/jiangang/investments/股票观察_副本.xlsm')
table = data.sheets()[1]
...
for c in codes[8:]:
    ...
    fl = '/library/webserver/documents/stock1/' + c[0:6] + '.php'
...
for i in range(8,len(pm)):
    ...
    if ii == 0:
        if float(yc[i+1]) < 0.1:
            ii = ii + 1
            f = codecs.open('/library/webserver/documents/stock1/px.php','w', 'UTF-8')
            px =  ah.replace('blog',pagename).replace("股筛子",'排序') + hybt1.replace("预计涨跌排序",'<a   href="/stock1/qpx.php" >排序（大于10%）</a>') + nr + '<table><br/><br/></table>' + at
            f.write(px)
            f.close()

f = codecs.open('/library/webserver/documents/stock1/qpx.php','w', 'UTF-8')
...
for h in hy1list:
    ...
    nr = ah.replace('blog','行业1 ' + h).replace("股筛子",h)  \
                ...
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/library/webserver/documents/stock1/'+ hh['行业1' + ' ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
for h in hy2list:
    nr = '<p align="center">'
    ...
    nr = ah.replace('blog','行业2 ' + h).replace("股筛子",h)  \
                ...
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/library/webserver/documents/stock1/' + hh['行业2' + ' ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
for h in hy3list:
    nr = '<p align="center">'
    ...
    nr = ah.replace('blog','行业3 ' + h).replace("股筛子",h)  \
                ...
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/library/webserver/documents/stock1/' + hh['行业3' + ' ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
for h in hy4list:
    nr = '<p align="center">'
    ...
    nr = ah.replace('blog','行业4 ' + h).replace("股筛子",h)  \
                ...
                + nr + '</table>' + '<br/><br/>' + at
    f = codecs.open('/library/webserver/documents/stock1/' + hh['行业4 ' + h] + '.php','w', 'UTF-8')
    f.write(nr)
    f.close()
print("        各个分行业排序文件完成。")
...
for i in range(3,len(c1)):
    hy = hy + '<tr><td align="center" width="4%">' \
            ...
            + tp(c11[i]) + '</td></tr>'
f = codecs.open('/library/webserver/documents/stock1/hy.php','w', 'UTF-8')
...
##    f = codecs.open('/library/webserver/documents/stock1/' + fname + '.php','w', 'UTF-8')
